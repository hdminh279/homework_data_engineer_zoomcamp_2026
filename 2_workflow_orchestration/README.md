...
```sql
SELECT COUNT(1) 
FROM `zoomcamp-kestra-2026.trips_data_all.yellow_tripdata` 
...
```sql
SELECT COUNT(1)
FROM `zoomcamp-kestra-2026.trips_data_all.green_tripdata` 
...
```sql
SELECT COUNT(1) 
FROM `zoomcamp-kestra-2026.trips_data_all.yellow_tripdata_2021_03`;
```
...
**Answer:**  
- Add a `timezone` property set to `America/New_York` in the `Schedule` trigger configuration (see in 02_workflow_orchestration.yaml)
